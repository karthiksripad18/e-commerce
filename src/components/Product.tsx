...
const Product = ({ id, title, price, category, description, image }: { id: number, title: string, price: string, category: string, description: string, image: string}) => {
    ...
    return (
        <div className="text-primary font-limelight container shadowed-2xl rounded bg-white flex items-center justify-evenly">
            <div className="ml-2">
            ...
            <Modal isOpen={modalIsOpen}>
                <div className="w-full h-full flex flex-col relative font-mono">
                    ...
                    <div className="w-full h-full flex flex-col md:flex-row justify-evenly items-center">
                        <img className="h-48 w-18 md:h-76 w-64 rounded" src={image} alt={title} />
                        <p className="ml-2 w-full md:w-1/2 text-sm md:text-2xl">{description}</p>
                    </div>
                </div>
            </Modal>
        </div>
